# file: app_v3.py
"""
Diário de Bordo Contemporâneo v3.0.0 — A4 Landscape
Atualizações (v3.0.0, calibradas com folha real de uso de 07/07 — v1.1.13):
  - Cabeçalho: "DIA #" substituído por seletor de dia da semana
    (SEG-DOM com caixas), pois o uso real mostrou o dia escrito à mão.
  - Priorização: prefixo "YYMMDD-" substituído por campo ID
    (uso real mostrou códigos tipo WTDMF-2730) e o Ouro ganhou uma
    linha REF: dedicada (código de referência era escrito por cima
    das caixas de pomodoro).
  - Pomodoros reduzidos: Ouro 12→8, Prata 8→6, Bronze 6, Cobre 4
    (caixas em excesso não eram usadas).
  - Restaurado tratamento de erro e mensagem de sucesso no __main__.
Herdado da v2.2.0: layout em 3 colunas, Recorrentes da Semana,
Blocos do Dia horários, ★ Amanhã Começa Com no rodapé da coluna,
Bloqueios, Notas (tags @), Fechamento Rápido, Hábitos + Pomodoros Totais.
"""
...
def draw_header(c, cursor):
    ...
    # ── Seletor de dia da semana (uso real: dia escrito à mão no "DIA #") ────
...
def draw_sections(c, cursor):
    ...
    # ── Tarefa 1: OURO (ID + REF + pomodoros) ────────────────────────────────
    c.setFillColor(ORANGE_LIGHT)
    c.setStrokeColor(ORANGE)
    c.rect(fo_x, fo_y, fo_w, 56, fill=1, stroke=1)

    c.setFont(FB, 8)
    c.setFillColor(ORANGE)
    c.drawString(fo_x + 5, fo_y + 44, "1. OURO — ID:")
    linha_start = fo_x + 5 + c.stringWidth("1. OURO — ID:", FB, 8) + 4
    c.setStrokeColor(ORANGE)
    c.setLineWidth(0.5)
    c.line(linha_start, fo_y + 42, fo_x + fo_w - 10, fo_y + 42)

    c.setFont(FB, 7)
    c.drawString(fo_x + 5, fo_y + 27, "REF:")
    linha_start = fo_x + 5 + c.stringWidth("REF:", FB, 7) + 4
    c.line(linha_start, fo_y + 25, fo_x + fo_w - 10, fo_y + 25)

    c.setFont(FB, 7)
    c.setFillColor(ORANGE)
    c.drawString(fo_x + 5, fo_y + 8, "POMODOROS:")
    px = fo_x + c.stringWidth("POMODOROS:", FB, 7) + 8
    draw_checkbox_group(c, px, fo_y + 6, count=8, numbered=True)

    def prio_task(num, nome, py, n_pomo):
        c.setFont(FB, 8)
        c.setFillColor(TEXT)
        label_str = f"{num}. {nome} — ID:"
