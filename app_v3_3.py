# file: app_v3_3.py
"""
Diário de Bordo Contemporâneo v3.3.0 — A4 Landscape
Atualizações (v3.3.0, calibradas com folha v3.2.0 impressa e anotada à mão):
  - Blocos do Dia estendidos de 08-18h para 05-22h: a folha ganhou
    "05-06h" e "06-07h" manuscritos acima do bloco, "07-08h" escrito
    sobre a barra de título, "12-13h" espremido entre 11-12h e 13-14h,
    e "18-19h", "19-20h", "20-21h" e "21-22h" anotados no rodapé da
    coluna (17 blocos no total).
  - ★ Amanhã Começa Com movido da coluna 2 para o rodapé da coluna 3:
    o bloco original foi riscado com X na folha e o título reescrito à
    mão dentro da coluna do Fechamento Rápido, com um quadro desenhado
    abaixo do "DIA NO ALVO?".
  - Alturas recalculadas: coluna 2 agora é prio (200) + blocos (216);
    coluna 3 é bloqueios (110) + notas (122) + fechamento (116) +
    ★ amanhã (48), fechando em COL_H = 426.
Herdado da v3.2.0: REF inline nos pomodoros, prioridade 5. ESTANHO,
seletor de dia da semana (SEG-DOM), layout em 3 colunas da v2.2.0.
"""
...
def draw_header(c, cursor):
    ...
    dia_x = MARGIN + CONTENT_W - 215
    c.setFont(FB, 9)
    c.drawString(dia_x, y - title_bar_h + 8, "DIA:")
    wx = dia_x + c.stringWidth("DIA:", FB, 9) + 5
    for d in dias:
        cb(c, wx, y - title_bar_h + 7, size=7)
        c.setFont(FB, 5.5)
        c.setFillColor(WHITE)
        c.drawString(wx + 9, y - title_bar_h + 9, d)
        wx += 9 + c.stringWidth(d, FB, 5.5) + 7

    c.setFont(FB, 9)
    label_sem = "SEMANA #"
    sem_x = MARGIN + CONTENT_W - 325
...
def draw_sections(c, cursor):
    ...
    # ═════════ COLUNA 2: PRIORIZAÇÃO (5 NÍVEIS) + BLOCOS ═════════
    prio_h = 200
    yi_prio = box(
    ...
    def ref_field(px, py, fim, cor):
        """REF: inline à direita dos pomodoros (calibrado com folha anotada)."""
        c.setFont(FB, 7)
        c.setFillColor(cor)
        c.drawString(px, py, "REF:")
        c.setStrokeColor(cor if cor == ORANGE else LINE)
        c.setLineWidth(0.5)
        c.line(px + c.stringWidth("REF:", FB, 7) + 3, py - 1, fim, py - 1)

    # ── Tarefa 1: OURO (ID + pomodoros com REF inline) ───────────────────────
    ...
    c.drawString(fo_x + 5, fo_y + 28, "1. OURO - ID:")
    linha_start = fo_x + 5 + c.stringWidth("1. OURO - ID:", FB, 8) + 4
    c.setStrokeColor(ORANGE)
    c.setLineWidth(0.5)
    c.line(linha_start, fo_y + 26, fo_x + fo_w - 10, fo_y + 26)

    c.setFont(FB, 7)
    c.setFillColor(ORANGE)
    c.drawString(fo_x + 5, fo_y + 8, "POMODOROS:")
    px = fo_x + c.stringWidth("POMODOROS:", FB, 7) + 8
    px = draw_checkbox_group(c, px, fo_y + 6, count=10, numbered=True)
    ref_field(px + 6, fo_y + 8, fo_x + fo_w - 10, ORANGE)
    ...
    def prio_task(num, nome, py, n_pomo):
        ...
        px2 = draw_checkbox_group(c, px2, py - 17, count=n_pomo, numbered=True)
        ref_field(px2 + 6, py - 15, linha_fim, GRAY_DARK)

    prio_task(2, "PRATA - ID", fo_y - 14, 8)
    prio_task(3, "BRONZE - ID", fo_y - 48, 6)
    prio_task(4, "COBRE - ID", fo_y - 82, 4)
    prio_task(5, "ESTANHO - ID", fo_y - 116, 4)

    # ── Blocos Horários (Timeboxing) 05-22h, ocupando o resto da coluna
    blk_y = cursor - prio_h - GAP
    blk_h = COL_H - prio_h - GAP
    yi_blk = box(c, x2, blk_y, w2, blk_h, "Blocos do Dia (Timeboxing & Ritmo)", fs=7, color=BLUE_DARK)

    periodos = [
        "05-06h:", "06-07h:", "07-08h:", "08-09h:", "09-10h:",
        "10-11h:", "11-12h:", "12-13h:", "13-14h:", "14-15h:",
        "15-16h:", "16-17h:", "17-18h:", "18-19h:", "19-20h:",
        "20-21h:", "21-22h:"
    ]
    ...
    # ═════════ COLUNA 3: BLOQUEIOS, NOTAS, METACOGNIÇÃO E ★ AMANHÃ ═════════
    tmr_h = 48
    blq_h = 110
    ...
    fch_h = COL_H - blq_h - not_h - tmr_h - 3 * GAP
    ...
    # ── ★ Amanhã Começa Com (movido da coluna 2 para o rodapé da coluna 3,
    #    conforme risco e reescrita manuscrita na folha v3.2.0)
    tmr_y = fch_y - fch_h - GAP
    c.setFillColor(BLUE_LIGHT)
    c.setStrokeColor(BLUE_DARK)
    c.setLineWidth(1.0)
    c.rect(x3, tmr_y - tmr_h, w3, tmr_h, fill=1, stroke=1)
    c.setFillColor(BLUE_DARK)
    c.rect(x3, tmr_y - 14, w3, 14, fill=1, stroke=0)
    c.setFillColor(WHITE)
    c.setFont(FB, 7.5)
    c.drawString(x3 + 5, tmr_y - 10, "★ AMANHÃ COMEÇA COM:")
    tit_w = c.stringWidth("★ AMANHÃ COMEÇA COM:", FB, 7.5)
    c.setFont(FI, 6)
    c.drawString(x3 + 5 + tit_w + 4, tmr_y - 10, "(Decida agora, blinde sua rotina)")

    c.setStrokeColor(LINE)
    c.setLineWidth(0.45)
    c.setDash(2, 3)
    c.line(x3 + 5, tmr_y - 35, x3 + w3 - 5, tmr_y - 35)
    c.setDash()
...
def draw_habits_and_footer(c, cursor):
    ...
    hx = draw_checkbox_group(c, hx, hy - 1, count=20, numbered=True)
